...
class DistortionDetector(nn.Module):
    """
    A Vision Transformer (ViT) based model to detect lens distortion coefficients.
    ...
    def __init__(self, pretrained=True):
        ...
        # Replace the final classification head to output 5 distortion coefficients.
        # [k1, k2, k3, p1, p2]
        num_ftrs = self.backbone.heads.head.in_features
        self.backbone.heads.head = nn.Sequential(
            nn.Linear(num_ftrs, 1024), nn.ReLU(), nn.Dropout(0.2), nn.Linear(1024, 5)
        )

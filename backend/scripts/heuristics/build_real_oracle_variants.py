#!/usr/bin/env python3
"""Build legit image-backed oracle variants from scored CSV + ZIP inputs.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate real image-backed oracle ZIP variants with deterministic rule sets."
    )
    parser.add_argument(
        "--input-manifest",
        type=Path,
...
def main() -> None:
    ...
    for margin in safe_margins:
        ...
        variants.append(
            (
                f"real_oracle_safe_guard_m{str(margin).replace('.', 'p')}",
                selection,
    ...
    for margin in aggr_margins:
        ...
        variants.append(
            (
                f"real_oracle_aggr_guard_m{str(margin).replace('.', 'p')}",
                selection,
    ...
    if safe_margins and aggr_margins:
        ...
        variants.append(
            (
                f"real_oracle_combo_safe{str(safe_margins[-1]).replace('.', 'p')}_aggr{str(aggr_margins[-1]).replace('.', 'p')}",
                combo_selection,
                {
    ...
    for short_name, selection, rule_meta in variants:
        artifact_name = f"submission_{short_name}_{args.tag}"
        out_zip = args.out_dir.expanduser().resolve() / f"{artifact_name}.zip"
        id_source_map = args.out_dir.expanduser().resolve() / f"id_list_{short_name}_{args.tag}.csv"
    ...
    manifest = {
        ...
        "method": "build_real_oracle_variants.v1",
    ...
    manifest_path = args.manifest_dir.expanduser().resolve() / f"real_oracle_variants_{args.tag}_manifest.json"

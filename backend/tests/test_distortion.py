...
def test_undistort_image_with_camera_matrix():
    ...
    corrected_image, info = undistort_image(image, distortion_coeffs, camera_matrix=camera_matrix)
...
def test_undistort_image_grayscale():
    ...
    assert len(corrected_image.shape) == 2 or (len(corrected_image.shape) == 3 and corrected_image.shape[2] == 1)
